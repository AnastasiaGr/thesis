 ...
 # create a mirror of the TIMIT directory structure
  ...
  # (cd $TIMIT ; find ${DIR} -type f -name S[IX]\*WAV) | sort > ${DIR}.WAV
   ...
   # rm -f $DIR.convert
     ...
     #generate the acoutic feature vectors
    #HCopy -C config1 -S Mytest_Matlab/MYTEST.convert
   ...
   # sort wordlist | sed "s/'em/\\\'em/" > ${DIR}wordlist
    ...
    #create phone level MLF
   #HLEd -n monophones -S TEST1.PHN -G TIMIT -i TEST1Mono.mlf monotimit.led
   #sort monophones > ${DIR}monophones
...
# cat TRAINmonophones TESTmonophones | sort -u > total_monophones 
# paste total_monophones total_monophones > dict_phones


#Build phone network
#HLEd -S TRAIN.TXT -i TRAINTxt.mlf monotimit.led
#HLStats -T 1 -b TRAINbigfn -o -I TRAINTxt.mlf -S TRAIN.TXT total_monophones
#HBuild -T 1 -n TRAINbigfn total_monophones TRAINphone_network
